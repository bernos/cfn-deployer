		 ...
		 if (err) {
		 	...
		 	if (master) {
			 	...
			 	cf.createStack(params, function(err, data) {
			 		if (err) {
			 			console.error("Problem creating stack", err)
			 		} else {
...
function upload(s3, bucket, key, stream, cb) {
    ...
    .on('httpUploadProgress', function(e) {
        console.log("Uploading %s Part %d - %d of %d.", key, e.part, e.loaded, e.total);
    })
    .send(cb);
}
...
function waitForStack(cf, stackName, status, cb) {
    ...
    console.log("Waiting for stack %s to reach status %s.", stackName, status);
    ...
    function checkStatus(stackName, status, cb) {
        cf.describeStacks ({
            ...
            //      eventLogger.stop();
                    ...
                    _.delay(checkStatus, 1000, stackName, status, cb);
                }
            }
        })
    }
