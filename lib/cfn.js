...
/**
 * Updates a cloud formation stack and waits for the stack the reach
 * CREATE_COMPLETE status.
 *
 * @param {object} params
 * @param {function} cb
 */
function updateStack(params, cb) {
	var cf = new AWS.CloudFormation();
	
	cf.updateStack(params, function(err, data) {
 		err ? cb(["Failed to update stack", err]) 
			: waitForStack(cf, params.StackName, 'UPDATE_COMPLETE', cb);
 	});
}
